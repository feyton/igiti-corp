...
class District(models.Model):
    ...
    ecelogical_zone = models.CharField(max_length=255, choices=ecological_zones, default='Mid')
...
class TypesOfSeed(models.Model):
    # CHOICES
    types       = (('ORT', 'Orthodox'), ('RECAL', 'Recalcitrant'), ('SUB-OR', 'Sub-Orthodox'))
    storage     = (
        ('RT', 'Room temperature'),
    ...
    storage_type    = models.CharField(_('storage type'),max_length=500, choices=storage)
...
class SeedPretreatment(models.Model):
    ...
    types           = models.CharField(max_length=255, choices=types_choice, default='MC')
...
class SeedProduct(models.Model):
    ...
    common_name             = models.CharField(_('common name'), max_length=255, blank=True)
    ...
    category                = models.CharField(max_length=3, choices=category_choice, help_text='Choose one')
...
class OrderItem(models.Model):
    ...
    def get_total_seedlings(self):
        ...
        if self.item.germination_rate and self.item.seeds_kg:
            number = int(self.item.germination_rate) * int(self.item.seeds_kg) / 100
...
class Payment(models.Model):
    ...
    mobile_number    = models.CharField(_('mobile number'), max_length=13, blank=True, null=True)
